  ...
  try {
    ...
    for (let i = 0; i < DisplayMovies.length; i++) {
      ...
      for (let j = 0; j < pageResults.length; j++) {
        ...
        try {
          const existingDisplayMovie = await DisplayMovie.findOne({
            title,
            releaseYear,
          });
...
const cleanGenres = async(req,res) =>{
    try {
...
const cleanDisplayMovies = async(req,res) =>{
    try {
...
const populateGenres = async (req, res) => {
    try {
      const genresCodes = await getGenres();
...
module.exports = { addMoviesToDB, cleanGenres, populateGenres, cleanDisplayMovies };
